...
/*   Updated: 2025/08/01 17:52:32 by madel-va         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_handle_config_line(char *line, int fd, t_config *config)
{
	...
	{
		return (printf("Empty line detected, skipping.\n"), free(line), 1);
	}
...
static int	ft_process_config_line(char *line, int fd, t_config *config,
	int *map_found)
{
	int	status;
...
int	ft_parse_config(int fd, t_config *config)
{
	...
	while (ret > 0)
	{
		if (!ft_process_config_line(line, fd, config, &map_found))
			return (0);
		ret = get_next_line(fd, &line);
	}
	free(line);
...
// !He leido que está bien que se transformen las tabulaciones en 4 
// !espacios, no sé si tamb se aplica en el mapa
